...
for line in File:
	...
	col8 = var[8].split(':')[1:]; new_col8 = 'GT:' + ':'.join(col8); 
	sys.stdout.write(new_col8); # replace AlleleCount by Genotype 
	...
	for i in xrange(9,len(var)):
		...
		if poolsize > 0: 
			...
			if genotype[0] == '.': 
				...
				if refsum < 0: print >>sys.stderr, "poolsize is smaller than allele counts, please provide correct pool size, terminating program"; sys.exit();
			...
			if genotype[0] == '.': 
				...
				if len(GV) > pool_sizes[i-9]: print >>sys.stderr, "poolsize is smaller than allele counts, please provide correct pool size, terminating program"; sys.exit();
